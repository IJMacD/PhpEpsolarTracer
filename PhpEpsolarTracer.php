...
class PhpEpsolarTracer
{
	...
	private function convertData($respArray,$doubleBytes=Array(),$negBytes=Array()) {
		...
		for ($i = 0; $i<(count($respArray)/2); $i++) {
			...
			if (in_array($i,$doubleBytes)) {
				...
				$x = intval($dataH.$data, 16);

				if ($negByte && $x > 0x7FFFFFFF) {
					$resultArray[] = $x - 0x100000000;
				}
				else {
					$resultArray[] = $x;
				}
	...
	public function getInfoData() {
		...
		$object_count = ord($result[$offset++]);

		for ($i = 0; $i < $object_count; $i++) {
			$id = ord($result[$offset++]);
			$len = ord($result[$offset++]);
			$this->infoData[$id] = substr($result, $offset, $len);
			$offset += $len;
		}
